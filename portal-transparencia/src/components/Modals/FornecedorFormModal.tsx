...
export function FornecedorFormModal({ isOpen, onClose, onSuccess, initialData }: FornecedorFormModalProps) {
  ...
  const validate = () => {
    const newErrors: {name?: string, document?: string} = {}
    if (!validationsService.isValidLength(name, 50)) {
      newErrors.name = 'O nome deve ter no máximo 50 caracteres.'
    }
    
    const docError = validationsService.validateDocument(document)
    if (docError) {
      ...
      <DialogContent className="sm:max-w-[420px] rounded-2xl overflow-hidden p-0 gap-0 border-x-0 border-b-0 sm:border-x sm:border-b sm:rounded-xl">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 space-y-4 bg-white dark:bg-gray-900">
          <div className="space-y-2">
            ...
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <User className={`h-4 w-4 ${errors.name ? 'text-red-400' : 'text-gray-400'}`} />
              </div>
              <Input
                value={name}
                onChange={e => {
                  ...
                  if (!validationsService.isValidLength(e.target.value, 50)) {
                    setErrors(prev => ({ ...prev, name: 'Tamanho máximo de 50 caracteres' }))
                  } else {
                ...
                className={`pl-9 h-11 rounded-xl focus-visible:ring-indigo-500 transition-colors ${errors.name ? 'border-red-500 focus-visible:ring-red-500 bg-red-50 dark:bg-red-900/10' : ''}`}
                required
              />
            </div>
            <div className="flex justify-between text-xs mt-1">
              <span className="text-red-500 font-medium">{errors.name}</span>
              <span className={`ml-auto font-medium ${name.length >= 50 ? 'text-red-500' : 'text-gray-500 dark:text-gray-400'}`}>
                {name.length}/50
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <FileText className={`h-4 w-4 ${errors.document ? 'text-red-400' : 'text-gray-400'}`} />
              </div>
              <Input
                value={document}
                onChange={e => {
                  const formatted = validationsService.formatDocument(e.target.value)
                  setDocument(formatted)
                  const partialError = validationsService.validateDocumentPartial(formatted)
                  setErrors(prev => ({ ...prev, document: partialError }))
                }}
                maxLength={18}
                placeholder="Ex: 12.345.678/0001-90"
                className={`pl-9 h-11 rounded-xl focus-visible:ring-indigo-500 transition-colors ${errors.document ? 'border-red-500 focus-visible:ring-red-500 bg-red-50 dark:bg-red-900/10' : ''}`}
                required
              />
            </div>
            <div className="flex justify-between text-xs mt-1">
              <span className="text-red-500 font-medium">{errors.document}</span>
            </div>
          </div>
